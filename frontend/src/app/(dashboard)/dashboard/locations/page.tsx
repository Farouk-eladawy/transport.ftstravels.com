...
const OsmMapPicker = dynamic(
  () => import("@/components/osm-map-picker").then((m) => m.OsmMapPicker),
      ...
      >
        <DialogContent className="border-border bg-popover text-foreground sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            {/* OpenStreetMap search + Leaflet preview */}
            <div className="space-y-2">
              <Label className="text-muted-foreground">
                Location on map (OpenStreetMap)
                <span className="ml-1 text-xs text-muted-foreground/60">(optional, free)</span>
              </Label>
              <PlacesAutocomplete
                value={formPlace}
                onChange={(place) => {
                ...
                type={dialog.level}
                placeholder={`Search for ${dialog.level} on the map...`}
              />
              {formPlace && (
                <>
                  ...
                  <OsmMapPicker
                    ...
                    onChange={(lat, lng) =>
                      setFormPlace((prev) =>
                        prev
                          ? { ...prev, lat, lng }
                          : { placeId: "", name: formName, formattedAddress: "", lat, lng }
                      )
                    }
                  />
                </>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <PlacesAutocomplete
                ...
                placeholder={`Search for ${editDialog.level} on the map...`}
              />
              {editPlace && (
                <>
                  <p className="text-xs text-muted-foreground">
